...
public abstract class LazySearchableCollectionPageModel<T> : SearchableCollectionPageModel<T>
    where T : class, IIdentifiable, IUpdateable<T>
{
    private readonly Dictionary<int, long> timeItemLastUpdated = new();

    protected LazySearchableCollectionPageModel(DatabaseManager databaseManager, PopupService popupService,
        int forwardLoadingCount) : base(databaseManager, popupService, forwardLoadingCount)
    {
    }
    ...
    protected override async void UpdateCollection()
    {
    ...
    protected virtual async Task<bool> TryUpdateItem(T item)
    {
        ...
        try
        {
            if (!timeItemLastUpdated.ContainsKey(item.Id) ||
                await HaveItemsFieldsChanged(cancellationToken.Token, item,
                    timeItemLastUpdated.GetValueOrDefault(item.Id, 0)))
    ...
    protected abstract Task<bool> HaveItemsFieldsChanged(CancellationToken cancellationToken, T item,
