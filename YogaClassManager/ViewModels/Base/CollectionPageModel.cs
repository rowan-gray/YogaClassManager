...
public abstract partial class CollectionPageModel<T> : BasePageModel where T : class, IIdentifiable, IUpdateable<T>
{
    ...
    [ObservableProperty] private ObservableCollection<T> displayedCollection;
    ...
    public CollectionPageModel(DatabaseManager databaseManager, PopupService popupService, int forwardLoadingCount,
        bool autoRetrieve = true)
    {
        ResetPageCommand = new Command(ResetPage);
        EndOfListCommand = new Command(EndOfList);
        CancelTasks = new Command(CancelTasksExecute);
        this.databaseManager = databaseManager;
        this.popupService = popupService;
        this.forwardLoadingCount = forwardLoadingCount;
        DisplayedCollection = new ObservableCollection<T>();
        if (autoRetrieve)
            RetrieveCollection();
    }
    ...
    {
        if (ScrollToIndex is not null) ScrollToIndex(this, new ScrollToIndexEventArgs(index, animate));
    }
    ...
    protected void OnScrollToItem(T item, bool animate)
    {
        ...
        if (!retrievedCollection.Exists(i => i.Id == item.Id)) return;
    ...
    {
        if (DisplayedCollection is null || retrievedCollection is null) return;
        ...
        if (deletedIds.Count > 0) RemoveItemsUsingId(deletedIds);
    ...
    private void EndOfList()
    {
        ...
        if (range.Count > 0) AddRange(DisplayedCollection, range);
    }
    ...
    protected void RemoveItemsUsingId(List<int> deletedItemsId)
    {
        if (deletedItemsId.Count > 0)
            ...
            {
                if (Selection?.Id == id) Selection = default;
    ...
    private void InsertNewPeopleIntoDisplay(List<T> newItems)
    {
        ...
        foreach (var newPerson in newItems)
        {
            ...
            if (index < DisplayedCollection.Count + newItems.Count) DisplayedCollection.Insert(index, newPerson);
        }
    }
