...
public abstract class SearchableCollectionPageModel<T> : CollectionPageModel<T>
    where T : class, IIdentifiable, IUpdateable<T>
{
    private string editableSearchQuery;

    private string lastSearchQuery;

    protected SearchableCollectionPageModel(DatabaseManager databaseManager, PopupService popupService,
        int forwardLoadingCount, bool autoRetrieve = true) : base(databaseManager, popupService, forwardLoadingCount,
        autoRetrieve)
    {
        SearchCollectionCommand = new Command(SearchCollectionCommandExecute);
    }
    ...
    public string CurrentSearchQuery
    {
        ...
        set
        {
            if (value == editableSearchQuery) return;
