...
public partial class MainPageModel : BasePageModel
{
    ...
    [ObservableProperty] private ObservableCollection<QuickActionItem> quickActions;

    [ObservableProperty] private string quote;

    private List<string> quotes;
    private readonly Random random = new();

    [ObservableProperty] private ObservableCollection<QuickActionItem> todaysClasses;
    ...
    public MainPageModel(DatabaseManager databaseManager, PopupService popupService)
    {
        ...
        CheckPassesCommand = new Command(CheckPassesCommandExecute);
        EditLastClassCommand = new Command(EditLastClassCommandExecute);
        HeaderPressedCommand = new Command(HeaderPressedCommandExecute);
        ResetPageCommand = new Command(PopulateTodaysClasses);
        QuickActions = new ObservableCollection<QuickActionItem>
        {
            new QuickActionItem(AddStudentCommand, "Add Student", "add_person.png"),
            new QuickActionItem(AddPassCommand, "Add Pass", "add_symbol.png"),
            new QuickActionItem(CheckPassesCommand, "Check Passes Remaining", "search.png"),
            new QuickActionItem(EditLastClassCommand, "Edit Previous Class", "edit_calendar.png")
        };
        TodaysClasses = new ObservableCollection<QuickActionItem>();
    ...
    {
        using var fileStream = await FileSystem.Current.OpenAppPackageFileAsync("quotes.txt");
    ...
    private string GetHeaderText()
    {
        ...
        if (currentTime.Hour > 10) return "Have a good day Tracy";
    ...
    private async void HeaderPressedCommandExecute()
    {
        ...
        {
            "noot_noot.mp3", "noot-noot.mp3", "noot-3453.mp3", "noot-3459.mp3", "noot-3457.mp3", "noot-3452.mp3",
            "noot-3451.mp3", "noot-3450.mp3", "noot-noot-and-laugh.mp3", "pingu-music.mp3"
        };

        var audioPlayer =
            AudioManager.Current.CreatePlayer(
                await FileSystem.OpenAppPackageFileAsync(soundFiles[random.Next(0, soundFiles.Count)]));
    ...
    private async void PopulateTodaysClasses()
    {
        ...
        try
        {
            try
            {
                ...
                foreach (var c in classes)
                {
                    ...
                    {
                        var classRoll =
                            await databaseManager.ClassesService.GetTodaysClassRollFromClassAsync(
                                CancellationToken.None, c);
                        command = new Command(async () =>
                            await NavigationService.NavigateTo(nameof(MarkRollPageModel), c, null, classRoll));
                        image = "edit_calendar.png";
                    }
                    catch (ArgumentException)
                    {
                        command = new Command(async () =>
                            await NavigationService.NavigateTo(nameof(MarkRollPageModel), c, null));
        ...
        catch (IOException)
        {
            ...
            await popupService.DisplayAlert("File not valid",
                "There was an error when attempting to read from the database.\nPlease check the location of the database in the settings.",
                "Okay");
    ...
    private async void CheckPassesCommandExecute()
    {
        ...
        if (student is not null)
        {
            ...
            try
            {
                ...
                try
                {
                    passes = await databaseManager.StudentsService.GetStudentsPasses(CancellationToken.None,
                        student.Id);
                }
                catch (TaskCanceledException)
                {
                ...
                else if (passes.Count > 1)
                {
                    ...
                    await popupService.DisplayAlert("Passes Remaining",
                        $"{student.FullName} has {sumation} total classes remaining " +
    ...
    private async Task<Student> SelectStudent(string message)
    {
        ...
        try
        {
            ...
            if (studentName is null) return null;

            List<Student> students;
            try
            {
                students = await databaseManager.StudentsService.SearchStudentsAsync(CancellationToken.None,
                    studentName);
            }
            catch (TaskCanceledException)
            {
            ...
            else if (students.Count > 1)
            {
                ...
                foreach (var student in students) studentNames.Add(student.FullName);

                var selectedStudentName =
                    await popupService.DisplayActionSheet("Select Student", "Cancel", null, studentNames.ToArray());

                if (selectedStudentName == "Cancel") return null;
