...
public partial class AddPassPageModel : ObservableObject
{
    ...
    [ObservableProperty] private CasualPass casualPass;

    [ObservableProperty] private DatedPass datedPass;

    private string defaultPassType = "Term Pass";

    [ObservableProperty] private PassAlteration newAlteration;
    ...
    [ObservableProperty] private ObservableCollection<string> passTypes;

    [ObservableProperty] private Student student;

    [ObservableProperty] private TermPass termPass;

    [ObservableProperty] private ObservableCollection<Term> terms;
    ...
    public Message PassCallbackParameter
    {
        set => PassCallback = (Action<Pass>)value.Parameter;
    }
    ...
    public Message StudentParameter
    {
        set
        {
            ...
            NewAlteration = new PassAlteration(-1, -1, 0, null);
        }
    }

    public Message ClassParameter
    {
        set => ClassRestriction = (ClassSchedule)value.Parameter;
    }

    public ClassSchedule ClassRestriction { get; set; }
    ...
    private async void RemoveAlterationCommandExecute(object obj)
    {
        ...
        if (await Shell.Current.DisplayAlert("Confirm", "Are you sure you want to remove this alteration?", "Yes",
                "No"))
        {
    ...
    private void AddAlterationCommandExecute(object obj)
    {
        ...
        NewAlteration = new PassAlteration(-1, CurrentPass.Id, 1, null);
        SaveCommand?.NotifyCanExecuteChanged();
    }
    ...
    {
        if (CurrentPass is null) return false;
    ...
    private async void Appearing()
    {
        try
        {
            Terms = new ObservableCollection<Term>(
                await databaseManager.TermsService.GetTermsAsync(CancellationToken.None));
        }
        catch (TaskCanceledException)
        {
            await popupService.DisplayAlert("Operation Cancelled", "The previous operation was cancelled!", "Ok");
        ...
        var pass = new Pass(-1, Student.Id, 0, new ObservableCollection<PassAlteration>());
        ...
        DatedPass = new DatedPass(pass, 10, DateOnly.FromDateTime(DateTime.Now),
