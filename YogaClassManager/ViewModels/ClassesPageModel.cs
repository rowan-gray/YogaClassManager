...
{
    [ObservableProperty] private ObservableCollection<ClassRoll> classRolls = new();
    ...
    public ClassesPageModel(DatabaseManager databaseManager, PopupService popupService) : base(databaseManager,
        popupService, 25)
    {
        AddClassCommand = new RelayCommand(addClassCommandExecute);
        EditClassScheduleCommand = new RelayCommand(EditClassSchedule);
        RemoveClassScheduleCommand = new RelayCommand(RemoveClassSchedule, CanRemoveClassSchedule);
        AddClassRollCommand = new RelayCommand(AddClassRollCommandExecute);
        UpdateClassRollCommand = new RelayCommand(UpdateClassRollCommandExecute, EditAndRemoveClassesCommandCanExecute);
        RemoveClassRollCommand = new RelayCommand(RemoveClassRollCommandExecute, EditAndRemoveClassesCommandCanExecute);
    }
    ...
    public RelayCommand RemoveClassScheduleCommand { get; }
    ...
    private async void RemoveClassSchedule()
    {
        if (await popupService.DisplayAlert("Confirm",
                $"Are you sure you would like to delete {Selection}?\nThis cannot be undone.", "Yes", "Cancel"))
        {
            try
            {
    ...
    {
        if (SelectedClassRoll is null) return;
    ...
    private async void RetrieveClassRolls()
    {
        ...
        {
            classRolls =
                await databaseManager.ClassesService.GetClassRollsFromClassAsync(CancellationToken.None, Selection);
        }
        catch (TaskCanceledException)
        {
            await popupService.DisplayAlert("Operation Cancelled", "The previous operation was cancelled!", "Ok");
    ...
    private async void RemoveClassRollCommandExecute()
    {
        ...
        if (await popupService.DisplayAlert("Confirm action",
                "Are you sure you want to delete this class. This cannot be undone!", "Yes", "Cancel"))
        {
            try
            {
    ...
    protected override void UpdateCollection()
    {
        ...
        if (true && Selection is not null) RetrieveClassRolls();
    }
    ...
    {
        return classesService.GetClassesAsync(cancellationToken.Token,
            returnArchivedClassSchedules: RetrieveArchivedClasses);
    }

    protected override Task<List<ClassSchedule>> GetUpdatedItems()
    {
        return classesService.GetUpdatedClassSchedule(cancellationToken.Token, timeLastUpdated,
            RetrieveArchivedClasses);
    }
