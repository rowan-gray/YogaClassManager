...
public class StudentsPageModel : LazySearchableCollectionPageModel<Student>
{
    ...
    private bool showInactiveStudents;

    public StudentsPageModel(DatabaseManager databaseManager, PopupService popupService) : base(databaseManager,
        popupService, 50)
    {
        EditDetailsCommand = new Command(EditDetailsCommandExecute);
        EditPassCommand = new Command(EditPassCommandExecute, PassCommandsCanExecute);
        AddPassCommand = new Command(AddPassCommandExecute);
        RemovePassCommand = new Command(RemovePassCommandExecute, PassCommandsCanExecute);
        UpdatePassCanExecute = new Command(UpdatePassCanExecutesCommand);
        EditEmergencyContactCommand =
            new Command(EditEmergencyContactCommandExecute, EditEmergencyContactCommandCanExecute);
        AddEmergencyContactCommand = new Command(AddEmergencyContactCommandExecute);
        RemoveEmergencyContactCommand =
            new Command(RemoveEmergencyContactCommandExecute, EditEmergencyContactCommandCanExecute);
        AddStudentCommand = new Command(AddStudentCommandExecute);
        RemoveStudentCommand = new Command(RemoveStudentCommandExecute, RemoveStudentCommandCanExecute);
        AddHealthConcernCommand = new Command(AddHealthConcernCommandExecute);
        RemoveHealthConcernCommand =
            new Command(RemoveHealthConcernCommandExecute, RemoveHealthConcernCommandCanExecute);
        AdvancedPassesViewCommand = new Command(AdvancedPassesViewExecute);
    }
    ...
    protected override async Task<bool> TryUpdateItem(Student item)
    {
        ...
        if (updated)
        {
            ...
            SelectedEmergencyContact = item?.EmergencyContacts.FirstOrDefault(emergencyContact =>
                emergencyContact.Id == selectedEmergencyContactId);
            SelectedHealthConcern =
                item?.HealthConcerns.FirstOrDefault(healthConcern => healthConcern == _selectedHealthConcern);
        }
    ...
    private async void RemovePassCommandExecute()
    {
        ...
        if (!await popupService.DisplayAlert("Confirm",
                $"Are you sure you want to remove the following pass?\n{SelectedPass}", "Yes", "No"))
            return;
        ...
        try
        {
            if (SelectedPass.ClassesUsed == 0)
            {
                ...
                {
                    await popupService.DisplayAlert("Operation Cancelled", "The previous operation was cancelled!",
                        "Ok");
                }
                catch (Exception e)
                {
            ...
            {
                if (!await popupService.DisplayAlert("Confirm",
                        "Some classes have already been redeemed off this pass.\n" +
                        "Would you like the pass to be altered to remove all remaining passes.", "Yes", "No"))
                    return;
                try
                {
                    await databaseManager.PassesService.AddPassAlterationAsync(cancellationToken.Token,
                        new PassAlteration(-1, SelectedPass.Id, -SelectedPass.ClassesRemaining,
                            "AUTOMATIC OPERATION: Pass Removed"));
                }
                catch (TaskCanceledException)
                {
                    await popupService.DisplayAlert("Operation Cancelled", "The previous operation was cancelled!",
    ...
    private async void RemoveEmergencyContactCommandExecute()
    {
        ...
        if (!await popupService.DisplayAlert("Confirm",
                $"Are you sure you want to remove the following emergency contact? \n{SelectedEmergencyContact.FullName}",
                "Yes", "No"))
            return;
        ...
        try
        {
            await databaseManager.StudentsService.RemoveEmergencyContactAsync(cancellationToken.Token,
    ...
    private async void RemoveHealthConcernCommandExecute()
    {
        ...
        if (!await popupService.DisplayAlert("Confirm",
                $"Are you sure you want to remove the following health concern? \n{SelectedHealthConcern}", "Yes",
                "No"))
            return;
        ...
        try
        {
            await databaseManager.StudentsService.RemoveHealthConcern(cancellationToken.Token, Selection,
                SelectedHealthConcern);
        }
        catch (TaskCanceledException)
        {
    ...
    private async void AddHealthConcernCommandExecute()
    {
        ...
        var healthConcerns =
            await popupService.DisplayPromptAsync("Health Concern", "What health concern would you like to add?");
        ...
        try
        {
            foreach (var value in healthConcerns.Split(", "))
            {
                ...
                await databaseManager.StudentsService.AddHealthConcern(cancellationToken.Token, Selection,
                    healthConcern.Substring(0, 1).ToUpperInvariant() + healthConcern.Substring(1));
            }
        }
        catch (TaskCanceledException)
        {
    ...
    {
        await NavigationService.NavigateToAddStudentPage(studentReturn: StudentAdded);
    }
    ...
    {
        return databaseManager.StudentsService.SearchStudentsAsync(cancellationToken.Token, query, null,
    ...
    {
        return studentsService.GetUpdatedStudentsMatchingQuery(cancellationToken.Token, timeLastUpdated,
            CurrentSearchQuery, ShowInactiveStudents);
    }
    ...
    protected override Task<bool> HaveItemsFieldsChanged(CancellationToken cancellationToken, Student item,
