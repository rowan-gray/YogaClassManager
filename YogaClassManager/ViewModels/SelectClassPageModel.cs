...
public partial class SelectClassPageModel : ObservableObject
{
    ...
    [ObservableProperty] private ObservableCollection<ClassSchedule> availableClasses;

    [ObservableProperty] private ObservableCollection<TermClassSchedule> selectedClasses;
    ...
    public SelectClassPageModel(DatabaseManager databaseManager, PopupService popupService)
    {
        ...
        SelectedClasses = new ObservableCollection<TermClassSchedule>();
        AddCommand = new Command(AddCommandExecute, () => selectedClasses.Count != 0);
        CancelCommand = new Command(CancelCommandExecute);
        SelectClassCommand = new Command(SelectClassCommandExecute);
        RemoveClassCommand = new Command(RemoveClassCommandExecute);
        ChangeClassCountCommand = new Command(ChangeClassCountCommandExecute);
    }

    public Message CallbackParameter
    {
        set => Callback = (Action)value.Parameter;
    }

    public Message TermParameter
    {
        set
        {
            Term = (Term)value.Parameter;
            RetrieveClasses();
        }
    }
    ...
    public void SelectClassCommandExecute(object obj)
    {
        ...
        if (AvailableClasses.Remove(classSchedule))
        {
            ...
            SelectedClasses.Add(new TermClassSchedule(classSchedule, count, 0));
        }
    ...
    public async void ChangeClassCountCommandExecute(object obj)
    {
        ...
        var response = await popupService.DisplayPromptAsync("Change Class Count",
            $"Please enter the new class count for {termClassSchedule.ClassSchedule}.",
        ...
        if (response is not null)
            try
            {
                ...
                if (classCount == 0) throw new ArgumentException($"Cannot have a class count of {classCount}");
                termClassSchedule.ClassCount = classCount;
            ...
            {
                await popupService.DisplayAlert("Incorrect format",
                    "There was an error converting your input into a number.\nPlease try again.", "OK");
            }
    ...
    public async void RetrieveClasses()
    {
        try
        {
            AvailableClasses = new ObservableCollection<ClassSchedule>(
                await databaseManager.ClassesService.GetClassesAsync(CancellationToken.None,
                    Term.Classes.Select(termClass => termClass.ClassSchedule).ToList()));
        }
    ...
    public async void AddCommandExecute()
    {
        try
        {
            await databaseManager.TermsService.SaveTermsClassesAsync(CancellationToken.None, Term.Id,
