...
public class TermsPageModel : LazySearchableCollectionPageModel<Term>
{
    private bool retrieveCompletedTerms;

    private TermClassSchedule selectedClass;

    public TermsPageModel(DatabaseManager databaseManager, PopupService popupService) : base(databaseManager,
        popupService, 25)
    {
    ...
    private async void RemoveClassCommandExecute()
    {
        if (!await popupService.DisplayAlert("Confirm",
                $"Are you sure you want to remove {SelectedClass.ClassSchedule} from the term?", "Yes", "No"))
            return;

        try
        {
            await databaseManager.TermsService.RemoveClassAsync(cancellationToken.Token, Selection,
    ...
    protected override async Task<List<int>> GetDeletedIds()
    {
        return await termsService.GetDeletedTermsAsync(cancellationToken.Token, timeLastUpdated);
    }

    protected override Task<bool> HaveItemsFieldsChanged(CancellationToken cancellationToken, Term item,
