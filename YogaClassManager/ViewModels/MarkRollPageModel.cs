...
public partial class MarkRollPageModel : ObservableObject, IQueryAttributable
{
    ...
    [ObservableProperty] private ClassSchedule? classSchedule;
    ...
    [ObservableProperty] private ObservableCollection<Student> searchedStudents;

    [ObservableProperty] private string searchFeedback;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    {
        ...
        SearchStudentsCommand = new Command(SearchStudentsCommandExecute);
        AddStudentCommand = new Command(AddStudentCommandExecute);
        RemoveStudentCommand = new Command(RemoveStudentCommandExecute);
        SaveCommand = new RelayCommand(SaveCommandExecute, SaveCommandCanExecute);
        CancelCommand = new RelayCommand(CancelCommandExecute);
        SaveCommandCanExecuteChangedCommand = new RelayCommand(SaveCommandCanExecuteChanged);
        UpdatePassCommand = new Command(UpdatePass);
        MarkedStudents = new ObservableCollection<ClassRollEntry>();
        SearchedStudents = new ObservableCollection<Student>();
    ...
    public async void ApplyQueryAttributes(IDictionary<string, object> query)
    {
        if (!queryAttributesApplied)
        {
            ...
            if (query.ContainsKey("parameter3"))
            {
                ...
                foreach (var entry in classRoll.StudentEntries) entry.Pass.ClassesUsed -= 1;
                Date = classRoll.Date;
            ...
            else
            {
                ...
                //try
                //{
                classRollId = await databaseManager.ClassesService.AddClassRollAsync(CancellationToken.None,
                    new ClassRoll(-1, Date, ClassSchedule, new List<ClassRollEntry>()));
                //}
    ...
    private async void SaveCommandExecute()
    {
        ...
        if (Date.DayOfWeek != ClassSchedule?.Day)
            if (!await popupService.DisplayAlert("Confirm",
                    $"The selected date does not fall on a {ClassSchedule?.Day}.\nAre you sure you would like save?",
                    "Yes", "No"))
                return;

        //try
        //{
        await databaseManager.ClassesService.UpdateClassRollDetailsAsync(CancellationToken.None,
            new ClassRoll(classRollId, Date, ClassSchedule, MarkedStudents.ToList()));
        //}
        //catch (TaskCanceledException)
        //{
        //    await popupService.DisplayAlert("Operation Cancelled", "The previous operation was cancelled!", "Ok");
    ...
    private async void CancelCommandExecute()
    {
        if (await popupService.DisplayAlert("Confirm", "Are you sure you want to cancel this sign in?", "Yes", "No"))
        {
            if (initialClassRoll is null)
                await databaseManager.ClassesService.RemoveClassRollAsync(CancellationToken.None,
                    new ClassRoll(classRollId, Date, ClassSchedule, MarkedStudents.ToList()));
            else
    ...
    private async void RemoveStudentCommandExecute(object obj)
    {
        ...
        //try
        //{
        await databaseManager.ClassesService.RemoveStudentFromClassRollAsync(CancellationToken.None, classRollId,
            classRollEntry);
        //}
        //catch (TaskCanceledException)
        //{
    ...
    private async void UpdatePass(object obj)
    {
        ...
        //try
        //{
        await databaseManager.ClassesService.UpdateClassRollEntryAsync(CancellationToken.None, classRollId,
            classRollEntry);
        //}
        //catch (TaskCanceledException)
        //{
    ...
    private async Task AddStudentToRoll(Student student)
    {
        ...
        if (studentToAdd?.Passes?.Count == 0)
        {
            if (await popupService.DisplayAlert("Insufficient Passes",
                    ...
                    "Yes", "No"))
                await NavigationService.NavigateToAddPassPage(Student.Copy(studentToAdd), StudentPassAdded, null,
                    classSchedule);
            return;
        }
...
#pragma warning disable CS8602 // Dereference of a possibly null reference.
        ...
        if (currentSelectedPass is null)
        {
            ...
            foreach (var pass in studentToAdd?.Passes)
            {
                if (!pass.IsExpired && !options.Contains("Use another pass")) options.Add("Use another pass");
                if (pass.IsExpired && !options.Contains("Use expired pass")) options.Add("Use expired pass");

                if (options.Contains("Use another pass") && options.Contains("Use expired pass")) break;
            }

            var response = await popupService.DisplayActionSheet(
                $"{studentToAdd.FullName} has no valid passes.\nWhat would you like to do?",
                "Cancel",
                null,
                options.ToArray());
            ...
            if (response == "Use another pass")
            {
                var pass = await popupService.DisplayActionSheet("Select pass", "Cancel", null,
                    studentToAdd.Passes.Where(p => !p.IsExpired).Select(p => p.ToString()).ToArray());

                if (pass is null || pass == "Cancel") return;
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        await databaseManager.ClassesService.AddStudentToClassRollAsync(CancellationToken.None, classRollId,
            classRollEntry);
        SearchedStudents.Remove(studentToAdd);
        MarkedStudents.Add(new ClassRollEntry(studentToAdd, currentSelectedPass));
        Search = null;
        RequestSearchBarFocus.Invoke(this, new EventArgs());
        //}
        //catch (Exception ex)
        //{
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    private async void SearchStudentsCommandExecute(object searchQuery)
    {
        ...
        //try
        //{
        var _students = await databaseManager.StudentsService.SearchStudentsAsync(CancellationToken.None, search,
            markedStudents.Select(markedStudent => markedStudent.Student), greedyLoad: false, limit: 25);

        if (_students.Count == 1 && _students.FirstOrDefault()?.FirstName.Length < search.Length &&
            _students.FirstOrDefault()?.FullName.ToLower().StartsWith(search.ToLower()) == true)
        {
            SearchFeedback = "No more students returned";
            SearchedStudents = new ObservableCollection<Student>(_students);
