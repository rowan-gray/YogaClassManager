...
public class ClassesService : DatabaseService
{
    ...
    public async Task<List<ClassSchedule>> GetClassesAsync(CancellationToken cancellationToken,
        List<ClassSchedule> excludedClasses = null, bool returnArchivedClassSchedules = false)
    {
        ...
        if (!returnArchivedClassSchedules) query += "AND IsActive = 0";
    ...
    internal async Task<List<ClassSchedule>> GetTodaysClassesAsync(CancellationToken cancellationToken,
        bool returnArchivedClassSchedules = false)
    {
        var query = "SELECT ClassScheduleId, Day, Time, IsActive FROM ClassSchedule\n" +
                    $"WHERE Day = {(int)DateTime.Now.DayOfWeek}";

        if (!returnArchivedClassSchedules) query += "\nAND IsActive = 0";
    ...
    {
        var query = "UPDATE ClassSchedule " +
    ...
    internal async Task<int> AddClassAsync(CancellationToken cancellationToken, ClassSchedule classSchedule)
    {
        ...
        var query = "INSERT INTO ClassSchedule " +
    ...
    {
        var query = "SELECT null FROM ClassSchedule " +
    ...
    internal async Task<List<ClassRoll>> GetClassRollsFromClassAsync(CancellationToken cancellationToken,
        ClassSchedule selectedClass)
    {
        ...
        {
            classRoll.ClassSchedule =
                await dbManager.ClassesService.GetClassFromIdAsync(cancellationToken, classRoll.ClassSchedule.Id);
            classRoll.StudentEntries =
                await dbManager.ClassesService.GetClassRollsStudentEntriesFromClassRollId(cancellationToken,
                    classRoll.Id);
        }
    ...
    internal async Task<ClassRoll> GetLastClassRollAsync(CancellationToken cancellationToken)
    {
        ...
        if (result is null) throw new ArgumentException("There is no previous class");
        ...
        classRoll.ClassSchedule =
            await dbManager.ClassesService.GetClassFromIdAsync(cancellationToken, classRoll.ClassSchedule.Id);
        classRoll.StudentEntries =
            await dbManager.ClassesService.GetClassRollsStudentEntriesFromClassRollId(cancellationToken, classRoll.Id);

        return classRoll;
    }

    internal async Task<ClassRoll> GetTodaysClassRollFromClassAsync(CancellationToken cancellationToken,
        ClassSchedule classSchedule)
    {
        ...
        if (result is null) throw new ArgumentException("There is no class roll today");
    ...
    internal async Task<ClassSchedule> GetClassFromIdAsync(CancellationToken cancellationToken, int id)
    {
        ...
        if (result is null) throw new ArgumentException($"There is no class schedule with id {id}");
    ...
    internal async Task<List<ClassRollEntry>> GetClassRollsStudentEntriesFromClassRollId(
        CancellationToken cancellationToken, int classId)
    {
        ...
        foreach (var result in results)
        {
            ...
            classRollEntries.Add(new ClassRollEntry(student, pass));
        }
    ...
    internal async Task UpdateClassRollEntryAsync(CancellationToken cancellationToken, int classRollId,
        ClassRollEntry classRollEntry)
    {
        await dbManager.BeginTransactionAsync();
        try
        {
    ...
    internal async Task AddStudentToClassRollAsync(CancellationToken cancellationToken, int classRollId,
        ClassRollEntry classRollEntry)
    {
        await dbManager.BeginTransactionAsync();
        try
    ...
    internal async Task RemoveStudentFromClassRollAsync(CancellationToken cancellationToken, int classRollId,
        ClassRollEntry classRollEntry)
    {
        await dbManager.BeginTransactionAsync();
        try
    ...
    {
        var query = "SELECT null FROM ClassRoll " +
                    $"WHERE ClassId = {id}";
    ...
    internal async Task<List<ClassSchedule>> GetUpdatedClassSchedule(CancellationToken cancellationToken,
        long timestamp, bool returnInactiveClass = false)
    {
        ...
        if (!returnInactiveClass) query += " AND IsActive = 0";
