...
public class StudentsService : DatabaseService
{
    ...
    public async Task<Student> GetStudentAsync(CancellationToken cancellationToken, int id, bool greedyLoad = false,
        bool returnInactivePasses = false)
    {
        var result = (await dbManager.Database.QueryAsync<StudentMapping>(
            "SELECT PersonId, FirstName, LastName, PhoneNumber, Email, IsActive FROM Person " +
            "JOIN Student ON Person.PersonId = Student.StudentId " +
            $"WHERE PersonId = {id}")).FirstOrDefault();

        if (result is null) throw new ArgumentException($"Student with id {id} does not exist");

        var student = result.ToStudent();

        if (greedyLoad) await PopulateStudentsFields(cancellationToken, student, returnInactivePasses);

        return student;
    }

    public async Task<List<Student>> GetStudentsAsync(CancellationToken cancellationToken,
        bool returnInactiveStudents = false, bool greedyLoad = false, bool returnInactivePasses = false)
    {
        var result = await dbManager.Database.QueryAsync<StudentMapping>(
            "SELECT PersonId, FirstName, LastName, PhoneNumber, Email, IsActive FROM Person " +
            "JOIN Student ON Person.PersonId = Student.StudentId " +
        ...
        var students = result.Select(result => result.ToStudent()).ToList();
        ...
        int? limit = null)
    {
        ...
        if (limit is not null) selectQuery += $" LIMIT {limit}";
    ...
    {
        var result = await Database.QueryAsync<EmergencyContactMapping>(
            "SELECT Person.PersonId, FirstName, LastName, PhoneNumber, Email, IsActive, " +
        ...
        var emergencyContacts = result.Select(result => result.ToEmergencyContact()).ToList();

        return emergencyContacts;
    }

    public async Task<List<Pass>> GetStudentsPasses(CancellationToken cancellationToken, int id,
        bool retrieveExpiredPasses = false, bool retrieveFulfilledPasses = false)
    {
        var datedPasses =
            await GetStudentsDatedPasses(cancellationToken, id, retrieveExpiredPasses, retrieveFulfilledPasses);
        var termPasses =
            await GetStudentsTermPasses(cancellationToken, id, retrieveExpiredPasses, retrieveFulfilledPasses);
    ...
    private async Task<List<CasualPass>> GetStudentsCasualPasses(CancellationToken cancellationToken, int id,
        bool retrieveFulfilledPasses)
    {
        ...
        if (!retrieveFulfilledPasses) query += " AND TimesUsed < NumberOfClasses";
        ...
        foreach (var result in results)
        {
            ...
            var pass = new Pass(result.PassId, result.StudentId, result.TimesUsed,
                new ObservableCollection<PassAlteration>(alterations));
    ...
    public async Task<List<TermPass>> GetStudentsTermPasses(CancellationToken cancellationToken, int id,
        bool retrieveExpiredPasses, bool retrieveFulfilledPasses)
    {
        ...
        if (!retrieveExpiredPasses) query += " AND Term.EndDate >= date('now')";
        ...
        foreach (var result in results)
        {
            ...
            var termClassSchedule =
                term.Classes.FirstOrDefault(termClassSchedule => termClassSchedule.ClassSchedule.Id == result.ClassId);
            var alterations = await dbManager.PassesService.GetAlterationsFromPassId(cancellationToken, result.PassId);
    ...
    public async Task<List<DatedPass>> GetStudentsDatedPasses(CancellationToken cancellationToken, int id,
        bool retrieveExpiredPasses, bool retrieveFulfilledPasses)
    {
        ...
        if (!retrieveExpiredPasses) query += " AND DatedPass.EndDate >= date('now')";
        ...
        foreach (var result in results)
        {
            ...
            var pass = new Pass(result.PassId, result.StudentId, result.TimesUsed,
                new ObservableCollection<PassAlteration>(alterations));
            var datedPass = new DatedPass(pass, result.ClassCount, StringToDateOnly(result.StartDate),
                StringToDateOnly(result.EndDate));
    ...
    public async Task<List<string>> GetStudentsHealthConcernsAsync(CancellationToken cancellationToken, int id)
    {
        ...
        foreach (var result in results) healthConcerns.Add(result.HealthConcern);
    ...
    internal async Task AddEmergencyContactAsync(CancellationToken cancellationToken, EmergencyContact emergencyContact)
    {
        ...
        try
        {
            ...
            var query = "INSERT INTO StudentEmergencyContacts (StudentId, EmergencyContactId, Relationship)\n" +
    ...
    internal async Task RemoveEmergencyContactAsync(CancellationToken cancellationToken,
        EmergencyContact emergencyContact)
    {
        var query = "DELETE FROM StudentEmergencyContacts " +
    ...
    internal async Task UpdateEmergencyContactAsync(CancellationToken cancellationToken,
        EmergencyContact emergencyContact)
    {
        var query = "UPDATE StudentEmergencyContacts " +
    ...
    {
        var query = "INSERT INTO StudentHealthConcerns " +
    ...
    {
        var query = "DELETE FROM StudentHealthConcerns " +
                    $"WHERE StudentId={student.Id} AND healthConcern='{healthConcern.Replace("'", "''")}'";

        await Database.ExecuteAsync(query);
    }

    internal async Task<List<Student>> GetUpdatedStudents(CancellationToken cancellationToken, long timestamp,
        bool returnInactiveStudents = false, bool greedyLoad = false, bool retrieveInactivePasses = false)
    {
        ...
        if (!returnInactiveStudents) query += " AND IsActive = 1";

        var results = await Database.QueryAsync<StudentMapping>(query);
        var students = results.Select(result => result.ToStudent()).ToList();
    ...
    internal async Task<List<Student>> GetUpdatedStudentsMatchingQuery(CancellationToken cancellationToken,
        long timestamp, string searchQuery, bool returnInactiveStudents = false, bool greedyLoad = false,
        bool retrieveInactivePasses = false)
    {
    ...
    private async Task<bool> HavePassesChanged(CancellationToken cancellationToken, Student student, long timestamp)
    {
        ...
        if (results2.Count > 0) return true;
        ...
        if (results3.Count > 0) return true;
    ...
    private async Task<bool> HaveEmergencyContactsChanged(CancellationToken cancellationToken, Student student,
        long timestamp)
    {
    ...
    private async Task<bool> HaveHealthConcernsChanged(CancellationToken cancellationToken, Student student,
        long timestamp)
    {
        var query = "SELECT null\n" +
                    "FROM Modifications M\n" +
    ...
    internal async Task<Student> PopulateStudentsFields(CancellationToken cancellationToken, Student student,
        bool retrieveExpiredPasses = false, bool retrieveFulfilledPasses = false)
    {
        student.EmergencyContacts =
            new ObservableCollection<EmergencyContact>(
                await GetStudentsEmergencyContacts(cancellationToken, student.Id));
        student.Passes = new ObservableCollection<Pass>(await GetStudentsPasses(cancellationToken, student.Id,
            retrieveExpiredPasses, retrieveFulfilledPasses));
        student.HealthConcerns =
            new ObservableCollection<string>(await GetStudentsHealthConcernsAsync(cancellationToken, student.Id));
        return student;
    }
}