...
public class TermService : DatabaseService
{
    ...
    public async Task<List<Term>> GetTermsAsync(CancellationToken cancellationToken,
        bool retrieveCompletedTerms = false, bool greedyLoad = true)
    {
    ...
    public async Task<Term> GetTermFromIdAsync(CancellationToken cancellationToken, int id, bool greedyLoad = true)
    {
        ...
        if (result is null) throw new ArgumentException($"No term exists with id {id}");

        var term = result.ToTerm();

        if (greedyLoad) await PopulateTermsFields(cancellationToken, term);
    ...
    {
        var query = "SELECT null FROM Term " +
                    $"WHERE TermId = {id}";
    ...
    internal async Task AddClassAsync(CancellationToken cancellationToken, int termId,
        TermClassSchedule termClassSchedule)
    {
        var query = "INSERT OR REPLACE INTO TermClasses " +
                    $"VALUES ({termId},{termClassSchedule.ClassSchedule.Id}, {termClassSchedule.ClassCount})";

        await Database.ExecuteAsync(query);
    }

    internal async Task SaveTermsClassesAsync(CancellationToken cancellationToken, int termId,
        List<TermClassSchedule> termClassSchedules)
    {
        ...
        try
        {
            var query = "REPLACE INTO TermClasses VALUES\n";
    ...
    {
        var query = "DELETE FROM TermClasses " +
                    $"WHERE TermId={term.Id} AND ClassId={classSchedule.Id}";

        await Database.ExecuteAsync(query);
    }

    internal async Task<List<Term>> GetUpdatedTermsAsync(CancellationToken cancellationToken, long timestamp,
        bool retrieveCompletedTerms = false, bool greedyLoad = true)
    {
        var query = "SELECT TermId, TermName, StartDate, EndDate, CatchupStartDate, CatchupEndDate\n" +
