...
public class PassesService : DatabaseService
{
    ...
    {
        var query = "SELECT null FROM Pass " +
                    $"WHERE PassId = {id}";
    ...
    internal async Task SavePassAsync(CancellationToken cancellationToken, Pass pass)
    {
        ...
        try
        {
            var query = "DELETE FROM PassAlterations " +
            ...
            {
                var updatePassQuery = "UPDATE TermPass " +
            ...
            else if (pass.GetType() == typeof(DatedPass))
            {
                ...
                var updatePassQuery = "UPDATE DatedPass " +
    ...
    internal async Task AddPassAlterationAsync(CancellationToken cancellationToken, PassAlteration alteration)
    {
        ...
        var query = "INSERT INTO PassAlterations " +
                    "VALUES (?,?,?,?)";
    ...
    {
        var query = "SELECT null FROM PassAlterations " +
    ...
    internal async Task<List<PassAlteration>> GetAlterationsFromPassId(CancellationToken cancellationToken, int passId)
    {
        ...
        var alterations = results.Select(mapping => mapping.ToPassAlteration()).ToList();
    ...
    internal async Task<Pass> GetPassFromId(CancellationToken cancellationToken, int passId)
    {
        ...
        {
            var datedPassAlterations =
                await dbManager.PassesService.GetAlterationsFromPassId(cancellationToken, datedPassResult.PassId);
            var datedPass = new Pass(datedPassResult.PassId, datedPassResult.StudentId, datedPassResult.TimesUsed,
                new ObservableCollection<PassAlteration>(datedPassAlterations));
            return new DatedPass(datedPass, datedPassResult.ClassCount, StringToDateOnly(datedPassResult.StartDate),
                StringToDateOnly(datedPassResult.EndDate));
        }
        ...
        {
            var casualPassAlterations =
                await dbManager.PassesService.GetAlterationsFromPassId(cancellationToken, casualPassResult.PassId);
            var casualPass = new Pass(casualPassResult.PassId, casualPassResult.StudentId, casualPassResult.TimesUsed,
                new ObservableCollection<PassAlteration>(casualPassAlterations));
        ...
        if (termPassResult is null) throw new ArgumentException($"There does not exist a pass with id {passId}");

        var term = await dbManager.TermsService.GetTermFromIdAsync(cancellationToken, termPassResult.TermId);
        var termClassSchedule = term.Classes.FirstOrDefault(termClassSchedule =>
            termClassSchedule.ClassSchedule.Id == termPassResult.ClassId);
        var alterations =
            await dbManager.PassesService.GetAlterationsFromPassId(cancellationToken, termPassResult.PassId);

        var pass = new Pass(termPassResult.PassId, termPassResult.StudentId, termPassResult.TimesUsed,
