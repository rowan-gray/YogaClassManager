...
public class PeopleService : DatabaseService
{
    ...
    internal async Task<List<Person>> GetPeopleAsync(CancellationToken cancellationToken,
        bool retrieveHiddenPeople = false)
    {
    ...
    internal async Task<int> AddPersonAsync(CancellationToken cancellationToken, Person person)
    {
        ...
        try
        {
            ...
            await Database.ExecuteAsync(query, id, person.FirstName, person.LastName, person.PhoneNumber, person.Email,
                person.IsActive);
    ...
    {
        var query = "SELECT null FROM Person " +
                    $"WHERE PersonId = {id}";
    ...
    internal async Task SavePersonAsync(CancellationToken cancellationToken, Person person)
    {
        ...
        await Database.ExecuteAsync(query, person.FirstName, person.LastName, person.PhoneNumber, person.Email,
            person.IsActive);
    }
    ...
    internal async Task<List<Person>> SearchPeopleAsync(CancellationToken cancellationToken, string searchQuery,
        bool returnHiddenPeople = false)
    {
    ...
    internal async Task<List<Person>> GetUpdatedPeople(CancellationToken cancellationToken, long timestamp,
    ...
    internal async Task<List<Person>> GetUpdatedPeopleMatchingQuery(CancellationToken cancellationToken, long timestamp,
        string searchQuery, bool returnInactiveStudents = false)
    {
        var query = "SELECT PersonId, FirstName, LastName, PhoneNumber, Email, IsActive\n" +
                    "FROM Modifications M\n" +
                    "INNER JOIN Person P ON M.id_value = P.PersonId\n" +
