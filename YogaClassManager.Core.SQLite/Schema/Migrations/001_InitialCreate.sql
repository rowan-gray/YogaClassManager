-- Baseline schema, matching the production yogamanager.db.sql verbatim (tables already used
-- IF NOT EXISTS there; views/triggers gain it here so this script is a no-op against a database
-- that already has them, and a full bootstrap against a brand-new empty one).
...
CREATE VIEW IF NOT EXISTS PassUses AS
...
CREATE VIEW IF NOT EXISTS PassesTotalClasses AS
...
CREATE VIEW IF NOT EXISTS TermClassUses AS
